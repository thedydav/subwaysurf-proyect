...
@implementation SliderSwitch {
    ...
    float valueOfSlider;
    uint64_t sliderOffset;
}

- (id)initSliderNamed:(NSString *)hackName_ description:(NSString *)description_ minimumValue:(float)minimumValue_ maximumValue:(float)maximumValue_ sliderColor:(UIColor *)sliderColor_ offset:(uint64_t)offset_{
    preferencesKey = hackName_;
    switchValueKey = [hackName_ stringByApplyingTransform:NSStringTransformLatinToCyrillic reverse:false];
    description = description_;
    sliderOffset = offset_;
    ...
    switchLabel = [[UILabel alloc]initWithFrame:CGRectMake(20, 0, menuWidth - 60, 30)];
    switchLabel.text = [NSString stringWithFormat:@"%@ %.2f", hackName_, minimumValue_];
    switchLabel.textColor = switchTitleColor;
    switchLabel.font = [UIFont fontWithName:switchTitleFont size:18];
    switchLabel.adjustsFontSizeToFitWidth = true;
    switchLabel.textAlignment = NSTextAlignmentCenter;
...
-(void)sliderValueChanged:(UISlider *)slider_ {
    ...
    [self writeSliderValue:slider_.value];
}

-(void)writeSliderValue:(float)value {
    uint32_t valueBits;
    memcpy(&valueBits, &value, sizeof(valueBits));
    writeData32([menu getFrameworkName], sliderOffset, valueBits);
}
...
- (void)addOffsetSwitch:(NSString *)hackName_ description:(NSString *)description_ offset:(uint64_t)offset_ bytes:(NSString *)bytes_ {
    std::vector<uint64_t> offsets = {offset_};
    std::vector<std::string> bytes = {[bytes_ UTF8String]};
    OffsetSwitch *offsetSwitch = [[OffsetSwitch alloc] initHackNamed:hackName_ description:description_ offsets:offsets bytes:bytes];
    [menu addSwitchToMenu:offsetSwitch];
}

- (void)addSliderSwitch:(NSString *)hackName_ description:(NSString *)description_ minimumValue:(float)minimumValue_ maximumValue:(float)maximumValue_ sliderColor:(UIColor *)sliderColor_ offset:(uint64_t)offset_ {
    SliderSwitch *sliderSwitch = [[SliderSwitch alloc] initSliderNamed:hackName_ description:description_ minimumValue:minimumValue_ maximumValue:maximumValue_ sliderColor:sliderColor_ offset:offset_];
