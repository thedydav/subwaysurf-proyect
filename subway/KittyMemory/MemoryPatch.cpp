...
static bool isReadableRange(const void *address, size_t size)
{
  ...
  return (pageInfo.protection & VM_PROT_READ) != 0;
}
                         ...
                         const void *patch_code, size_t patch_size)
{
  ...
  if (!isReadableRange(reinterpret_cast<const void *>(_address), patch_size))
  {
    _address = 0;
    _size = 0;
    return;
...
MemoryPatch MemoryPatch::createWithHex(const char *fileName, uintptr_t address, std::string hex)
{
  ...
  if (!isReadableRange(reinterpret_cast<const void *>(patch._address), patch._size))
  {
    patch._address = 0;
    patch._size = 0;
    return patch;
